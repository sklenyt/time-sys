...
To je snadná past hlavně u spravovaných Postgres služeb a u oficiálního Docker image `postgres` — obojí typicky nastaví výchozí/bootstrap uživatele (`POSTGRES_USER`) jako superuser. Toto skutečně způsobilo pád e2e testu multi-tenant izolace v CI (viz [`.github/workflows/ci.yml`](../.github/workflows/ci.yml)): `postgres:16-alpine` service container udělá z `POSTGRES_USER` superusera, takže RLS politika reálně nikdy nezasáhla — test procházel ze špatného důvodu (žádná izolace).

Důležitá past navíc: **bootstrap uživateli (tomu, co `initdb` použil při založení clusteru) nejde superuser sebrat vůbec**, ani přes `ALTER ROLE ... NOSUPERUSER` od jiného superusera — Postgres to odmítne hláškou `The bootstrap user must have the SUPERUSER attribute`. Řešení proto není "snížit práva bootstrap uživatele", ale **založit úplně samostatnou, od začátku neprivilegovanou roli** pro appku a tou i spouštět migrace (aby jí patřily vytvořené tabulky). V CI to znamená nechat `POSTGRES_USER` u service containeru jako čistě bootstrapovací účet (`postgres`) a hned prvním krokem vytvořit aplikační roli `depo` s `NOSUPERUSER`, které se pak předá vlastnictví databáze/schématu — teprve tou se pouští `prisma migrate deploy` i testy.
...
- U spravovaných platforem (RDS, Cloud SQL, Render Postgres…) je výchozí "admin"/bootstrap účet skoro vždy superuser a nejde mu to sebrat — je potřeba založit samostatnou aplikační roli bez tohoto příznaku (`CREATE ROLE ... NOSUPERUSER`) a jí vlastnit/používat schéma, ne se pokoušet upravit ten výchozí účet.
