...
- ✅ **Výpočet výsledků + export XLSX** (F12, F13) — `GET /routes/:id/results`, on-the-fly z `zaznam_udalosti` (žádná zvlášť udržovaná tabulka, viz [04-data-model.md §4.5](04-data-model.md)): pořadí celkové i po kategoriích, časová penalizace zohledněna, DNS/DNF/DQ vyřazeni do samostatné sekce. Veřejné čtení bez přihlášení (F16 — základ pro živou stránku). `GET .../results/export.xlsx` stahuje stejná data jako sešit. Export PDF zatím chybí.
