...
## Fáze 4 — RFID, multi-tenant, reporty (viz `docs/10-roadmap.md`)

- `GET/POST /routes/:routeId/register` — **vlastní veřejný registrační formulář** (F23), bez přihlášení; startovní číslo se přiděluje automaticky (nejnižší volné, retry při souběhu). `PATCH /routes/:routeId` s `registraceUzavrena` řídí otevření/uzavření.
- **Multi-tenant Row-Level Security** (F24) — PostgreSQL RLS na tenant-scoped tabulkách, prosazené přes `TenantContextInterceptor` + Prisma middleware podle přihlášeného uživatele; defense-in-depth nad aplikační vrstvou.
- `GET /reports/course-records?nazev=`, `GET /reports/runner-history?prijmeni=&jmeno=` — **rekordy tratě a historie výkonů** (F26), scoped na vlastní organizaci jako `/events`.
- `GET /routes/:routeId/anomalies` — **podezřele rychlý/pomalý čas** (F33) oproti mediánu kategorie (robustní MAD, ne pevný práh).
- `GET /routes/:routeId/results/bezec/:prihlaskaId`, `.../qr.png` — **osobní výsledek + QR kód** na startovním čísle.
- **E-mail při doběhu** (F32) — volitelné `oznamovaciEmail` na přihlášce, `nodemailer` s volitelným SMTP (bez konfigurace se tiše přeskočí).
- `POST/GET /routes/:routeId/records/:recordId/foto` — **fotodůkaz sporného doběhu** (F41) z fotoaparátu zařízení, uložený na disk (`apps/api/uploads/zaznamy/`).
- `POST/DELETE /routes/:routeId/entries/:entryId/chip`, `POST /routes/:routeId/records/rfid` — **párování RFID čipu a ingest endpoint** (F22, F29) pro Local Capture Agent; ověřeno mock agentem bez potřeby reálného hardwaru. Samotný ovladač konkrétního výrobce decodéru zůstává mimo scope.

Ověřeno end-to-end i pro Fázi 4: registrace přes veřejný formulář včetně automatického přidělení čísla; RLS politiky blokují křížový přístup mezi organizacemi (ověřeno i to, že bez tenant kontextu — veřejné/systémové operace — zůstává vše průchozí); rekordy tratě správně agregují víc ročníků stejného názvu trasy a najdou correct celkový i kategoriový rekord; anomálie správně označí uměle vloženého odlehlého běžce (15 min oproti mediánu 40 min) a nedotknou se běžců v normálním rozptylu; QR kód se dá skutečně načíst v prohlížeči (cross-origin — vyžadovalo úpravu `Cross-Origin-Resource-Policy` v helmetu) a vede na správnou osobní stránku; e-mailové oznámení doručeno na reálný lokální SMTP test server (`aiosmtpd`) se správným obsahem; fotodůkaz nahraný a stažený zpět beze změny bajtů, zobrazený v UI kolizí; RFID ingest správně přeloží kód čipu na startovní číslo a vytvoří identický záznam jako ruční zápis, včetně chybových stavů (neznámý/odpárovaný čip → 404).
